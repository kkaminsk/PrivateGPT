...
The system SHALL produce MSIX packages with modern Windows version targeting, `.msix` file extension, and custom application icon used consistently across all platforms.
...
#### Scenario: Consistent icon across platforms
- **GIVEN** the PNG files in `icon/` are derived from the same source as the ICO file
- **WHEN** the application is built for any platform (Windows, macOS, Linux)
- **THEN** the application displays the same icon design
- **AND** the icon appears at appropriate resolutions for each platform
